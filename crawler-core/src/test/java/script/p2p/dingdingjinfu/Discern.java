package script.p2p.dingdingjinfu;
...
public class Discern implements Script {
    ...
    public List<Task> work(Crawler crawler, Job job) throws Exception {
        ...
        JSONObject json = JSONObject.parseObject(page.getContent().trim());
        if(json.getString("status").equals("n")){
            map.put("registered","0");
        }else if(json.getString("status").equals("y")){

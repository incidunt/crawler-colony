 ...
 */
public class List implements Script {
    ...
    public java.util.List<Task> work(Crawler crawler, Job job) throws Exception {
        ...
        java.util.List<Crawler> crawlerList = new ArrayList<>();
        for (String phone : Parser.html(page.getContent()).regex("\\d{11,11}")) {
            ...
            if (set.contains(phone)) {
                ...
                crawler.put("phone", phone);
                map.put("grade4", "" + grade(phone));
                crawlerList.add(nc);
            }
        }
        java.util.List<Task> list = new ArrayList<>();
        Task task = new Task(crawlerList, new Detail());
        list.add(task);
        return list;
    }
